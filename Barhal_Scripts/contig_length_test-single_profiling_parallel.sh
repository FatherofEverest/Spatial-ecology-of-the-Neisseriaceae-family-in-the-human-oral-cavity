...
DIR_CLE=$mainDIR/CONTIG_LENGTH_TEST
DIR_ContigsDB=$DIR_CLE/04_CONTIGS_DB
DIR_Mapping=$DIR_CLE/05_MAPPING
DIR_SinglePROF=$DIR_CLE/06_PROFILES/Concatenated_both_no_filter
...
ELAPSED="Profiling - no filter $(($SECONDS / 3600))hrs $((($SECONDS / 60) % 60))min $(($SECONDS % 60))sec"
