...
genes = pd.read_csv("08_PROFILE_SUMMARY/P_0622_Haemophilus_Aggregatibacter_TD-profile/bin_by_bin/H_parainfluenzae_str_M1C160_1_id_GCA_014931275_1/H_parainfluenzae_str_M1C160_1_id_GCA_014931275_1-gene_coverages.txt", sep='\t').set_index("gene_callers_id")

# Load up genome coverage data
genome = pd.read_csv("08_PROFILE_SUMMARY/P_0622_Haemophilus_Aggregatibacter_TD-profile/bins_across_samples/mean_coverage.txt", sep='\t')

# select only the bin of interest H_parainfluenzae_str_M1C160_1_id_GCA_014931275_1
genome = genome[genome['bins'].str.contains("H_parainfluenzae_str_M1C160_1_id_GCA_014931275_1")]  #.drop('bins', axis=1)

# Drop first column of dataframe
genome = genome.reset_index(drop=True)

# Subset metagenomes
metagenomes = genome[[col for col in genome.columns if col.startswith('TD_HC_HMP_')]]
...
metagenomes_of_interest = ['TD_HC_HMP_' + sample for sample in samples_of_interest if 'TD_HC_HMP_' + sample in metagenomes]
