...
putGenoName=$( grep "$assemblyID" $meningitidis_gonorrhoeae_projectMetadata | awk -F',' -v OFS="\t" '{print $1,$9,$12}' | sed -e 's/"//g' | sed -e 's/ /_/g' | sed -e 's/uncultured_//' | sed -e 's/\[//' | sed -e 's/\]//' | awk -F'\t' -v OFS="|" '{print $1,$2,$3}' | awk 'BEGIN{FS=OFS="_"}{ $1= substr($1,1,1)}1' | sed -e 's/_sp\./_sp/' | sed -E 's/(.*)\./\1POINT/' | sed -e "s/[^[:alnum:]|]/_/g" | sed -E 's/(.*)POINT/\1\./' | sed -e 's/_//' | sed -e 's/|/;/' |  sed -e 's/_.*;/;/' | sed -e 's/;/_str_/' | sed -e 's/|/_id_/' | sed -e 's/^.\{1\}/&_/' | sed -e 's/_str__id_/_str_NA_id_/' | sed -e 's/\./_/')
echo -e "$putGenoName\t$genomeNo" >> $MEN_GNO_nameConversions
done < $meningitidis_gonorrhoeae_rawNCBIid

# find HOMD ID
for hypo_hmt in `cat $meningitidis_gonorrhoeae_rawNCBIid | awk -F'_id_' '{print $2}' `
do
cat $seqidHOMD | grep "$hypo_hmt" | awk -v gca_id="$hypo_hmt" 'BEGIN{FS=OFS="\t"}{print $3"_"$4"_str_"$5"_id_"gca_id}' | sed -e's/ //g' | sed -e 's/_sp\./_sp_/' | awk 'BEGIN{FS=OFS="_"}{ $1= substr($1,1,1)}1' | awk '{if ($1 ~/HMT/) gsub("_sp","");print}' | awk -F'_id_' -v OFS="\t" '{print $NF,$0}' | sed -e 's/-/_/g' | grep 'HMT' | sed -e 's/HMT/sp_HMT_/' >> $MEN_GNO_newHOMDid
done
...
oldID=$( grep "$gcaID" $MEN_GNO_nameConversions | awk '{print $1}' )
newID=$( echo $line | awk '{print $2}' | sed -e 's/\./_/' )
sed -i "s/$oldID/$newID/" $MEN_GNO_nameConversions
done < $MEN_GNO_newHOMDid
...
for genome in `cat $meningitidis_gonorrhoeae_genomesID`
do
cat $MEN_GNO_nameConversions | grep "$genome" | awk -v workPath="$PWD" 'BEGIN{FS=OFS="\t"}{print $1,workPath"/04_CONTIGS_DB/"$2"-contigs.db"}' >> $MEN_GNO_GENOMES
done
